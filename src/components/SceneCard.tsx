...
function PromptBlock({ label, text, color, icon, onGenerate, generating, onOpenDialog }: {
  label: string; text: string; color: string; icon: React.ReactNode;
  onGenerate?: () => void; generating?: boolean; onOpenDialog?: () => void;
}) {
              ...
              <DropdownMenuTrigger asChild>
                <button
                  ...
                  style={{
                    ...
                    border: "1px solid hsl(152 69% 40% / 0.15)",
                  }}
                  onClick={e => e.stopPropagation()}
                >
                  <Zap size={10} /> Gerar <ChevronRight size={9} />
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-48">
                {onGenerate && (
                  <DropdownMenuItem onClick={onGenerate} disabled={generating}>
                    <ImageIcon size={12} className="mr-2 text-yellow-500" />
                    {generating ? "Gerando..." : "Imagem Nano"}
                  </DropdownMenuItem>
                )}
...
const SceneCard = memo(({ scene, index, defaultOpen = false, completed = false, onToggleComplete, onRegenerate, regenerating = false, onImageGenerated, scriptId }: SceneCardProps) => {
  ...
  const handleGenerate = async (promptText: string) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/nano-generate`, {
        ...
        body: JSON.stringify({ action: "generate_image", prompt: promptText, scene_index: index, enhance: true }),
      });

      if (res.status === 429) { toast.error("Rate limit — aguarde"); return; }
  ...
  const openGenerateDialog = (promptText: string) => {
    setDialogPrompt(promptText);
    setDialogOpen(true);
  };
  ...
  return (
    <>
      <div
        className={cn("glass rounded-2xl overflow-hidden transition-all duration-300 animate-slide-up", completed && "opacity-50")}
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <ChevronDown size={16} className={cn("text-muted-foreground/40 transition-transform duration-300", open && "rotate-180")} />
          </div>
        </button>
          ...
          <div className="px-5 pb-5 flex flex-col gap-4">
            ...
            {hasNano && !nanoIsNA && (
              <PromptBlock
                ...
                onOpenDialog={() => openGenerateDialog(scene.prompt_nano!)}
              />
            )}
            ...
            {scene.prompt_veo && scene.prompt_veo !== "null" && (
              <PromptBlock
                ...
                onOpenDialog={() => openGenerateDialog(scene.prompt_veo)}
              />
            )}
            {scene.prompt_veo_b && scene.prompt_veo_b !== "null" && (
              <PromptBlock
                label="VEO 3.1 — SHOT B" text={scene.prompt_veo_b} color={promptColors.veob}
                ...
                onOpenDialog={() => openGenerateDialog(scene.prompt_veo_b!)}
              />
            )}
            {scene.prompt_veo_alt && scene.prompt_veo_alt !== "null" && (
              <PromptBlock
                label="VEO 3.1 — PLANO B" text={scene.prompt_veo_alt} color={promptColors.veo_alt}
                ...
                onOpenDialog={() => openGenerateDialog(scene.prompt_veo_alt!)}
              />
            )}
            {scene.prompt_kling && scene.prompt_kling !== "null" && (
              <PromptBlock
                ...
                onOpenDialog={() => openGenerateDialog(scene.prompt_kling)}
              />
            )}
      ...
      <GenerateDialog
        ...
        sceneIndex={index}
        scriptId={scriptId}
        onJobCreated={() => onImageGenerated?.("", index)}
      />
    </>
  );
});
